...
class CarLights {
  ...
  init() {
    ...
    if (Array.isArray(this.colors)) {
      ...
      colorArray = [new THREE.Color(this.colors as any)];
    }
    ...
    material.onBeforeCompile = shader => {
      shader.vertexShader = shader.vertexShader.replace(
        ...
        typeof this.options.distortion === 'object' ? (this.options.distortion as Distortion).getDistortion : ''
      );
    };
  ...
  init() {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: Object.assign(
        ...
        this.webgl.fogUniforms,
        (typeof options.distortion === 'object' ? (options.distortion as Distortion).uniforms : {}) || {}
      )
    });
...
class App {
  ...
  onMouseUp(ev: MouseEvent | TouchEvent) {
    if (this.options.onSlowDown) this.options.onSlowDown(ev as any);
    this.fovTarget = this.options.fov;
    this.speedUpTarget = 0;
  }

  onTouchStart(ev: TouchEvent) {
    if (this.options.onSpeedUp) this.options.onSpeedUp(ev as any);
    this.fovTarget = this.options.fovSpeedUp;
    this.speedUpTarget = this.options.speedUp;
  }
  ...
  update(delta: number) {
    ...
    if (typeof this.options.distortion === 'object' && (this.options.distortion as Distortion).getJS) {
...
const Hyperspeed: FC<HyperspeedProps> = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }) => {
  ...
  useEffect(() => {
    ...
    const options: HyperspeedOptions = {
      ...defaultOptions,
      ...effectOptions,
      colors: { ...defaultOptions.colors, ...effectOptions.colors } as any
    };
